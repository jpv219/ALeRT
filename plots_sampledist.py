...
def plot_point_dist3d(in_idx_list,iter_list,ini_df,df,data_name):
...
def plot_point_dist2d(in_idx_list,ini_df,df,data_name):
    ...
    plt.scatter(x1, x2, marker='^', s=30, color = "blue",label=f'{data_name}')
...
def main():
    ...
    iter = input('Provide the the sample numbers in 3 iterations (e.g. 10,20,30): ')
    iter_list = [int(x) for x in iter.split(',')]

    PATH = PathConfig()

    ini_pkl = os.path.join(PATH.input_savepath, case_name, 'ini','X_train_i.pkl')
    random_pkl = os.path.join(PATH.resample_savepath, case_name,'random', f'X_random.pkl')

    if data_name == 'random':
        input_pkl = os.path.join(PATH.resample_savepath, case_name,data_name, f'X_{data_name}.pkl')
    else:
        input_pkl = os.path.join(PATH.resample_savepath, case_name,data_name, f'X_train_{data_name}.pkl')
    ...
    if len(in_idx_list) == 3:
        ...
        if plot_dimen.lower() == '2d':
            ...
            plot_point_dist2d(in_idx_list_sub1,ini_df,input_df,data_name)
            in_idx_list_sub2 = [in_idx_list[0], in_idx_list[2]]
            plot_point_dist2d(in_idx_list_sub2,ini_df,input_df,data_name)
            in_idx_list_sub3 = [in_idx_list[1], in_idx_list[2]]
            plot_point_dist2d(in_idx_list_sub3,ini_df,input_df,data_name)
        else:
            # define_random_center(in_idx_list,random_df) #35
            plot_point_dist3d(in_idx_list,iter_list,ini_df, input_df,data_name)
