...
def main():
    ...
    # data_choice = input('Select a dataset to process from the study selected above (random, dt, gsx): ')
    ...
    metrics_choice = input('Select the metric to plot (r2,mse,mae):')
    ...
    ini_y = df[ini_col]
    plt.scatter(ini_x,ini_y,label=r'Initial')
    ...
    plt.scatter(random_x,random_y,label=r'Random')

    dt_col = [col for col in plotting_col if 'dt' in col]
    dt_x = [int(col.split('_')[-1]) for col in dt_col]
    dt_y = df[dt_col]
    plt.scatter(dt_x,dt_y,label=r'DT-guided')

    # gsx_col = [col for col in plotting_col if 'gsx' in col]
    # gsx_x = [int(col.split('_')[-1]) for col in gsx_col]
    # gsx_y = df[gsx_col]
    # plt.scatter(gsx_x,gsx_y,label=r'GSx')
    plt.legend()
    plt.xlabel('Case Number')
    plt.ylabel(f'{metrics_choice}')
    fig.savefig(os.path.join(PATH.fig_savepath,case_name,f'{model_choice}_perf.png'))
